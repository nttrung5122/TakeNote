...
const noteController={
    ...
    getNote: async (req,res)=>{
        try {
            ...
            if(folder.havePassword && !password) {
                if(!req.body?.password){
                    return res.status(401).json("wrong password");
                }

                const validPassword = await bcrypt.compare(password,folder.password);
                if(!validPassword){
                    return res.status(401).json("wrong password");
                }
            }
            ...
            note.title = cryptoJsService.decrypt(note.title,password);
            note.content = cryptoJsService.decrypt(note.content,password);
            res.status(200).json(note);
        } catch (error) {
